...
@section('content')
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
            ...
            {{-- Columna Izquierda: Resumen + Contacto + Datos Bancarios --}}
            <div class="space-y-6">
                @include('checkout.includes.resumen_producto')
                @include('checkout.includes.contacto_whatsapp')
                @include('checkout.includes.datos_bancarios')
            </div>
            ...
            <div class="bg-white rounded-lg p-6 shadow-sm">
                ...
                <form action="{{ route('checkout.store') }}" method="POST" enctype="multipart/form-data" id="checkoutForm">
                    ...
                    <x-text-input name="phone_number" label="Número de teléfono" 
                    ...
                    <x-file-upload name="document_image" label="Imagen de Documento (DNI/Cédula)" 
                        accept="image/*" maxSize="5MB" allowedFormats="PNG, JPG" required />
                    
                    <x-file-upload name="payment_proof" label="Comprobante de Pago" 
                        accept="image/*" maxSize="5MB" allowedFormats="PNG, JPG" required />
